document.addEventListener('DOMContentLoaded', () => {
    ...
    if (noBtn) {
        noBtn.addEventListener('click', () => {
            moveCount++;
            if (moveCount < 3) {
                const x = Math.random() * Math.max(0, window.innerWidth - 170);
    ...
    window.pullGratitude = function pullGratitude() {
    ...
    setInterval(() => {
        ...
        daysEl.innerText = Math.floor(d / 86400000);
        hoursEl.innerText = Math.floor((d % 86400000) / 3600000);
        minsEl.innerText = Math.floor((d % 3600000) / 60000);
        secsEl.innerText = Math.floor((d % 60000) / 1000);
    }, 1000);

    window.sayYes = function sayYes() {
    ...
    window.startPhotoRain = function startPhotoRain() {
        if (typeof confetti === 'function') {
            confetti({
                particleCount: 150,
                spread: 70,
